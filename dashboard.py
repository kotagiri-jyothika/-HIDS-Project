...
# ═══════════════════════════════════════════════════════════════════════════════
   ...
   st.markdown("**WHY did the model flag this as an attack?**") st.markdown("---")  with st.expander(" What is SHAP? How do I read these charts? (Click to learn)", expanded=True):     st.markdown("""     ### SHAP in plain English      **SHAP** stands for **SHapley Additive exPlanations**.      Imagine you ask 5 people to solve a problem together.     SHAP figures out how much each person contributed to the final answer.     In our case, SHAP figures out how much each network feature     (like packet size, connection duration, number of errors)     contributed to the model's decision to flag something as an attack.      ---      ### How to read the SHAP summary plot      - **Each row = one feature** (a measurable property of network traffic)     - **Each dot = one network packet** from the test set     - **Colour** = the feature value (red = high value, blue = low value)     - **Position left/right** = how much it pushed the decision         - **Right side (positive)** = this feature pushed the model towards predicting ATTACK         - **Left side (negative)** = this feature pushed the model towards predicting NORMAL      ---      ### What the feature names mean      | Short Name | What It Actually Measures |     |---|---|     | src_bytes | How many bytes the source (attacker) sent |     | dst_bytes | How many bytes the destination (victim) received |     | duration | How long the connection lasted in seconds |     | count | How many connections to the same host in the last 2 seconds |     | serror_rate | Proportion of connections with SYN errors (sign of DoS attack) |     | rerror_rate | Proportion of connections with REJ errors |     | byte_ratio | Ratio of sent to received bytes — very high = suspicious |     | same_srv_rate | Proportion of connections to the same service |     | dst_host_count | Number of connections to the destination host |     | logged_in | Was the user logged in? (1=yes, 0=no) |      ---      ### Example reading      If **src_bytes** has many red dots on the RIGHT side, it means:     high amounts of data sent by the source = strong indicator of attack.     This makes sense for a DoS attack which floods targets with huge volumes of traffic.     """)
    st.markdown("---")
